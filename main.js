...
// =============================================================
// STEP 6 — WebXR AR MODE
//
// How it works:
//   - ARButton creates a standard "Enter AR" button and handles
//     the XRSession lifecycle automatically.
//   - When AR starts: hide Milky Way (camera feed is the background),
//     hide all 2D UI overlays (season label, slider, hints, tilt SVG),
//     disable OrbitControls (user moves phone instead of dragging),
//     scale the scene down so it fits on a table in the real world.
//   - When AR ends: restore everything exactly as it was.
//
// The scene is wrapped in an arSceneGroup so we can scale/position
// the whole solar system without touching individual object transforms.
// =============================================================
...
// AR scale: the solar system at normal size would be huge in a room.
// 0.01 makes 1 Three.js unit = 1 cm, so the orbit (~12 units across)
// fits on a typical desk as a ~24 cm wide display.
const AR_SCALE = 0.015;
const NORMAL_SCALE = 1.0;

// UI elements to hide in AR mode — only things that are meaningless or
// wrongly-positioned in AR (desktop drag/zoom instructions, and the
// tilt indicator, which is computed from the flat-screen camera projection).
// Season label, slider, and the info/eclipse UI stay available in AR.
const arHideEls = [
  "hint", "zoom-hint", "tilt-svg", "earthview-exit-btn",
].map(id => document.getElementById(id)).filter(Boolean);

let isARMode = false;
let preARCamPos = new THREE.Vector3();
let preARTarget = new THREE.Vector3();
...
function onARSessionStart() {
  ...
  // Hide desktop-only elements (drag/zoom hints, tilt indicator, earth-view exit)
  arHideEls.forEach(el => { el.dataset.arDisplay = el.style.display; el.style.display = "none"; });

  // season-info-btn / eclipse-demo-btn are normally revealed by finishZoomIn()
  // when zooming into Earth on desktop — but that flow is intentionally
  // disabled in AR (the tabletop model is already "up close"), so show them
  // directly instead.
  document.getElementById("season-info-btn").style.display = "flex";
  document.getElementById("eclipse-demo-btn").style.display = "block";

  // Disable OrbitControls — in AR the user moves the phone
  controls.enabled = false;

  // Save camera state to restore after AR
  preARCamPos.copy(camera.position);
  preARTarget.copy(controls.target);

  // Scale scene to tabletop size and position it slightly in front of camera
  arSceneGroup.scale.setScalar(AR_SCALE);
  // Place the solar system 0.5 metres in front of and 0.3m below eye level
  arSceneGroup.position.set(0, -0.3, -0.5);

  // Make background transparent so camera feed shows through
...
function onARSessionEnd() {
  ...
  arHideEls.forEach(el => { el.style.display = el.dataset.arDisplay || ""; });

  // Revert the buttons we manually showed for AR back to their normal
  // "overview" (hidden) state, and close any eclipse panel left open
  document.getElementById("season-info-btn").style.display = "none";
  document.getElementById("eclipse-demo-btn").style.display = "none";
  if (eclipsePanel.style.display === "block") hideEclipsePanel();
  hideInfoBubble();

  // Restore OrbitControls
  controls.enabled = true;
  camera.position.copy(preARCamPos);
  controls.target.copy(preARTarget);

  // Restore scene scale and position
  arSceneGroup.scale.setScalar(NORMAL_SCALE);
  arSceneGroup.position.set(0, 0, 0);

  // Restore opaque background
  renderer.setClearColor(0x000000, 1);
  scene.background = null; // milkyWaySphere handles the background visually
}

// Create and inject the AR button into the container div in index.html.
...
arButton.style.cssText = `
  ...
  pointer-events: auto; /* #ui-overlay sets pointer-events:none, so this must be re-enabled explicitly */
`;
